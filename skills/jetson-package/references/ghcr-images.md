...
|------------------------|-------------|
| `vllm` | `latest-jetson-orin` for Orin LLM/VLM serving; use upstream/native vLLM 0.20+ on Thor |
| `llama_cpp` | GGUF inference with GPU offload |
| `ollama` | Quick local GGUF chat |
| `live-vlm-webui` | Browser UI for VLMs |

Pick **Orin** vs **Thor** from **`JETSON_GENERATION`** after running `skills/jetson-diagnostic/scripts/detect_jetson.sh`. For vLLM, Orin maps to GHCR `latest-jetson-orin`; Thor should use upstream/native vLLM 0.20+ when available instead of a Jetson-specific container. **`JETSON_SKU`** remains the legacy bucket; **`JETSON_PRODUCT_LINE`** refines Orin to `orin-agx` / `orin-nx` / `orin-nano` and Thor today to **`thor-agx`**.
